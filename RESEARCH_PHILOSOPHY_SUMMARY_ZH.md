...
第十八节第九小节把表征与决策之间的岔口显式留了下来，并要求它成为一次决策记录而非隐性漂移。本节即是这次记录。

### 1. 决策

**代表作的一号贡献放在决策侧**：在观测预算约束下，系统如何知道自己不知道，并据此决定下一步应当观测什么。

表征侧（跨视角、变化中的地理配准三维语义如何被语言锚定）不被放弃，但降为支撑性贡献——它是让决策可被计算的前置条件，而不是论文要争夺的那面旗帜。

### 2. 判断依据：独有资产盘点

第十八节第九小节给出的判断依据是“押在自己拥有独有资产的那一侧”。据此盘点：

| 已沉淀的资产 | 性质 | 落在哪一侧 |
| --- | --- | --- |
| 跨视角当前数据条件下的两次 active-view 不可用判定 | 诚实的负面结论 | 决策 |
| 风险感知门控（仅在风险可判定时输出） | 弃权机制 | 决策 |
| 可审计智能体架构（出处、责任、可追溯） | 运行时保证 | 决策 |
| 修正后的诚实评估结果（剔除真值泄漏） | 评价方法论 | 决策 |
| 志愿地理信息的结构性空间偏差 | 覆盖度先验 | 决策 |

盘点结论是单侧的：**五项资产全部落在决策侧，表征侧为零。** 依据既已固定，结论就不应再被“哪一侧更容易发表”反向改写。

### 3. 代价与对冲

代价有三项，均已在第十八节预告：

1. **需要补动作代价模型。** 弃权若不接上代价，仍然只是空值。必须把平台能耗、单次通行机会、飞行小时、人员到场等约束显式建模，弃权才能转成“下一步观测什么”的建议。
2. **主战场迁移。** 未来数年的目标场域从遥感期刊转向机器人与视觉会议，短期内投稿命中率会下降。
3. **下游成熟度风险。** 非结构化户外自主若商业化晚到数年，纯押下游会被动。

对冲同样已经给出，且不依赖下游按时到达：

- **交付物层面**：可通行性与可达性今天就有人在环的消费者（抢修派工、调度、通行证据），机器人只是它后来的第二类消费者。同一产物横跨两个时代——下游晚到不致命，按时到达则免费放量。
- **平台层面**：动作代价模型的真实约束无法在纯软件环境中标定，需要具备多平台采集与真实硬件条件的研究环境。这一需求恰好与决策侧绑定：表征侧可以在公开数据集上独自完成，决策侧不能。因此对研究环境的选择应当以“是否提供真实动作约束”为首要判据，而非以表征侧算力与数据规模为判据。

### 4. 反向约束：四个在研项目的定位口径

决策一旦作出，即反向规定各项目对外的一号命题。此表用于阻止口径漂移。

| 项目 | 旧口径 | 决策侧口径 |
| --- | --- | --- |
| 跨视角评估 | 跨视角灾损评估的精度改进 | 观测配置的可用性判定与下一最佳视角触发 |
| 可审计智能体 | 负责任 GeoAI 与伦理保障 | 自主系统的运行时保证（runtime assurance） |
| 仲裁式评估 | 分类精度与一致性 | 证据充分性判定与拒答校准 |
| 空间偏差分析 | 数据质量与覆盖度描述 | 覆盖度感知的信念先验，用于指导观测调度 |

四行的共同变换是同一个：**输出类型从标签换成动作，或从等级换成信念。** 输入与方法骨架不变，可读性与消费者随之改变。

### 5. 一个新识别的接口：新类发现止步于分类

三维遥感中的新类发现（novel class discovery）与“先验失效”是同一问题的两个抽象层：前者问“训练集类别体系对新观测失效了吗”，后者问“先验地图对当前世界失效了吗”。

现有工作把这件事在**分类**处收口——发现新类、命名新类、报告发现率。它没有接到动作上：**发现先验失效之后，应当去哪里重新观测以修复它。**

这个缝隙的价值在于它不需要另起炉灶。方法论已被证明可行，路线图上的下一步在社区内是自明的，但没有人占住它。同理，“自主测图”在机器人社区的对应词就是 map maintenance，而 map maintenance 的核心难题恰是先验失效——**方向标签本身已经承认了这个问题的重要性，无需再论证。**

因此决策侧的入口不必表述为对既有工作的反对，而应表述为对既有工作的续接：把停在分类的结论接到观测调度上。

### 6. 失效条件

一次诚实的决策记录必须写明什么情况下应当撤销它。出现以下任一情形时，本节应被重新审议：

1. 动作代价模型在两个以上真实平台上都无法标定到可用精度，说明决策侧缺少物理基础。
2. 可通行性与可达性在人在环场景中找不到任何真实消费者，说明对冲不成立，下游风险重新变成裸露风险。
3. 弃权机制在扩大后的数据条件下不再稳定复现，说明独有资产的独有性不成立。

在此之前，本决策视为已定，不再逐次重新讨论。

> 一句话收口：把一号贡献放在“知道自己不知道、并据此决定下一步观测什么”，因为已沉淀的资产全部落在这一侧；表征降为支撑，代价由“同一产物横跨两个时代”对冲，口径由上表锁定。
